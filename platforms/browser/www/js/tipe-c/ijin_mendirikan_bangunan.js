tablename = "imb";
// var ijin_mendirikan_bangunan_new = {
//   path: '/tipe-c/ijin_mendirikan_bangunan_new/',
//   url: './pages/tipe-c/ijin_mendirikan_bangunan_new.html',
//   name: 'ijin_mendirikan_bangunan_new',
//   on: {
//     pageInit: function () {
...
//         $$("#operator_area").show();
//       }
//       $("#tanggal_pengajuan").prop('readonly', false);
//       $$("#addformupload").on("touchend", addrow);
//       // $$('#cek_nop').on('click', function () {
//         // app.input.validateInputs('#new_ijin_mendirikan_bangunan');
//         // if ($$('#new_ijin_mendirikan_bangunan')[0].checkValidity() == true) {
//         //   let data = [];
//         //   data.push(iamthedoor);
//         //   data.push($$('#nop').val());
//         //   app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/check_nop', data, function (response) {
//         //     if (response.status == 1) {
//         //       app.dialog.alert('Selamat Pembayaran SPPT Anda Telah Lunas, Silakan Melanjutkan Pembuatan IMB');
//               $$('#form_pemohon').show();
//               $$('#form_pengiriman').show();
//               $$('#form_bangunan').show();
//               $$('#cek_nop').hide();
//               $$('#simpan').show();
//           //   } else {
//           //     app.dialog.alert('Mohon Melunasi Pembayaran SPPT Anda Sebelum Melanjutkan Pembuatan IMB');
//           //   }
//           // }, function () {
...
//         if (this.value == 1) {
//           $$('#info_delivery').text('Peneng Dapat Diambil di Kecamatan Setelah SK Selesai');
//           $$('.form_penerima').hide();
//           $$('.form_penerima input').val('');
//           $$('.form_penerima textarea').val('');
//         } else if (this.value == 2) {
//           $$('#info_delivery').text('Peneng Akan Dikirim ke Alamat Anda Setelah SK Selesai dengan Biaya yang Dibebankan Kepada Anda Sebesar Rp. 15.000');
//           $$('.form_penerima').show();
//         }
//       });
...
//           });
//           mydata = app.form.convertToData("#new_ijin_mendirikan_bangunan");
//           data.push(mydata);
//           data.push(iamthedoor);
//           data.push(keteranganid);
//           data.push(filecode);
//           app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/save_ijin_mendirikan_bangunan', data, function (data) {
//             if (isNaN(data)) {
...
//   }
// };
var ijin_mendirikan_bangunan = {
  path: '/tipe-c/ijin_mendirikan_bangunan',
  url: './pages/tipe-c/ijin_mendirikan_bangunan.html',
  name: 'ijin_mendirikan_bangunan',
  on: {
    pageInit: function () {
      $$("#btnnew").hide();
      if (datauser.role_id == "4") {
      ...
      $$('#btnnew').on('click', function() {
        cordova.InAppBrowser.open("https://simbg.pu.go.id", '_system');
      });
      var statusselect = app.smartSelect.create({
        ...
        on: {
          close: function () {
            ...
            datatables.context[0].ajax.url = site_url_mobile_layanan + '/ijin_mendirikan_bangunan/layanan/' + $$('#statusselect').val();
      ...
      var datatables = $('#datatables').DataTable({
        "serverSide": true,
        "ajax": {
          "url": site_url_mobile_layanan + '/ijin_mendirikan_bangunan/layanan/1',
          "data": iamthedoor,
          "type": "GET"
        },
        "columns": [
        ...
        "rowCallback": function (row, data) {
          ...
          if (data.val_status == 'Menunggu') {
            if (datauser.role_id == '4') {
              $('td:eq(0)', row).html('<a href="/tipe-c/ijin_mendirikan_bangunan_edit/' + data.id + '/edit/" class="button button-small button-fill color-blue">' +
                '<i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Edit</a>');
            } else {
              $('td:eq(0)', row).html('<a href="/tipe-c/ijin_mendirikan_bangunan_edit/' + data.id + '/approve/" class="button button-small button-fill color-green">' +
                '<i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Approve</a>');
            }
          } else {
            $('td:eq(0)', row).html('<a href="/tipe-c/ijin_mendirikan_bangunan_edit/' + data.id + '/lihat/" class="button button-small button-fill color-green">' +
...
var ijin_mendirikan_bangunan_edit = {
  path: '/tipe-c/ijin_mendirikan_bangunan_edit/:id/:tipe',
  url: './pages/tipe-c/ijin_mendirikan_bangunan_edit.html',
  name: 'ijin_mendirikan_bangunan_edit',
  on: {
    pageInit: function () {
      tablename = "imb";
      $$("#addformupload").hide();
      ...
      $$("#print_button").on("click", function () {
        ...
        app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/print_doc/' + id, iamthedoor, function (doc_path) {
          download_doc(doc_path);
        }, 'json');
      });
      $$("#print_form_button").on("click", function () {
        app.dialog.preloader('Mohon Tunggu Sebentar...');
        app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/print_form/' + id, iamthedoor, function (doc_path) {
          download_doc(doc_path);
        }, 'json');
      });
      $$("#print_skrd_button").on("click", function () {
        app.dialog.preloader('Mohon Tunggu Sebentar...');
        app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/print_skrd/' + id, iamthedoor, function (doc_path) {
          download_doc(doc_path);
        }, 'json');
      });
      $$("#print_preview_button").on("click", function () {
        app.dialog.preloader('Mohon Tunggu Sebentar...');
        app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/print_invoice/' + id, iamthedoor, function (doc_path) {
      ...
      app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/find_layanan/' + id + '/' + datauser.bf_users_id, data, function (data) {
        if (data == false) {
          app.dialog.close();
          ...
          app.request.post(site_url_mobile + '/siurban_mobile/get_kecamatan_kelurahan/' + data.pemohon.kecamatan + '/' + data.pemohon.kode_desa, function (keckel) {
            ...
            $$('#delivery').val(data.layanan.delivery);
            if (data.layanan.delivery == 1) {
              $$('#info_delivery').text('Peneng Dapat Diambil di Kecamatan Setelah SK Selesai');
              $$('.form_penerima').hide();
            } else if (data.layanan.delivery == 2) {
              $$('#info_delivery').text('Peneng Akan Dikirim ke Alamat Anda Setelah SK Selesai dengan Biaya yang Dibebankan Kepada Anda Sebesar Rp. 15.000');
              $$('#nama_penerima').val(data.layanan.nama_penerima);
              $$('#telp_penerima').val(data.layanan.telp_penerima);
              $$('#alamat_penerima').text(data.layanan.alamat_penerima);
              $$('.form_penerima').show();
            }
            ...
            if ((tipe == 'edit' || tipe == 'lihat') && datauser.role_id == '4') {
              ...
              if (data.sum_approval.total < 3) {
                ...
                $$('.form_skrd').show();
                $$('#ijin_mendirikan_bangunan_edit input').prop("disabled", true);
                $$('#ijin_mendirikan_bangunan_edit textarea').prop("disabled", true);
            ...
            $$('#deletelayanan').on('click', function () {
              ...
              data = [];
              data.push(iamthedoor);
              app.request.post(site_url_mobile_layanan + '/ijin_mendirikan_bangunan/delete_layanan/' + id, data, function (data) {
                if (data == false) {
                  app.dialog.close();
                  app.dialog.alert('Data tidak ditemukan');
                  mainView.router.back();
      ...
      function prep_penyimpanan() {
        $$("#simpan").on("click", function () {
          ...
          if (datauser.role_id == '4') {
            ...
            mydata = app.form.convertToData("#edit_ijin_mendirikan_bangunan");
            data.push(mydata);
            data.push(iamthedoor);
            data.push(keteranganid);
            data.push(filecode);
            var url = site_url_mobile_layanan + '/ijin_mendirikan_bangunan/save_ijin_mendirikan_bangunan/update/' + id;
            ...
            if (this_user_is_the_last_index == true && $$("select[name='status']").val() == 2) {
              ...
              $$('#approve_button').on('click', function () {
                approve('/ijin_mendirikan_bangunan/save_ijin_mendirikan_bangunan/ustatus/' + id, this_user_is_the_last_index, $$('#esign').val());
                approval.close();
              });
            } else {
              var approval = app.popup.create({
              ...
              $$("#approve_button").on("touchend", function () {
                approve('/ijin_mendirikan_bangunan/save_ijin_mendirikan_bangunan/ustatus/' + id, this_user_is_the_last_index);
                approval.close();
              });
            }
          }
