      ...
      var datatables = $('#datatables').DataTable({
        ...
        "rowCallback": function (row, data) {
          ...
          if (is_user() && data.jenis_surat == 'usaha_kurma' && data.verified == 3) {
            $('td:eq(6)', row).html('<span style="background-color:#DE4E42; padding:5px; border-radius:10px; color:white;">Ditolak oleh Operator Verifikasi</span>');
          ...
          let edit_path = '/tipe-a/edit_sk_domisili_usaha/';
          if (data.jenis_surat == 'usaha_kurma') {
            edit_path = '/tipe-a/edit_sk_domisili_kurma/';
          }

          if (data.val_status == 'Menunggu' || (is_user() && data.val_status == 'Ditolak') || (is_user() && data.jenis_surat == 'usaha_kurma' && data.verified == 3)) {
            if (datauser.role_id == '4') {
              $('td:eq(0)', row).html('<a href="' + edit_path + data.id + '/edit/" class="button button-small button-fill color-blue">' +
                '<i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Edit</a>');
            } else {
              $('td:eq(0)', row).html('<a href="' + edit_path + data.id + '/approve/" class="button button-small button-fill color-green">' +
                '<i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Approve</a>');
            }
          } else {
            $('td:eq(0)', row).html('<a href="' + edit_path + data.id + '/lihat/" class="button button-small button-fill color-green">' +
          ...
          if (data.jenis_surat == 'usaha_kurma') {
            $('td:eq(7)', row).html('<a href="/tipe-a/tracking_sk_domisili_kurma/' + data.id + '/" class="button button-small button-fill color-orange">' +
              '<i class="icon f7-icons" style="font-size: 12pt;">zoom_in</i> Lihat</a>');
          } else {
            $('td:eq(6)', row).html('---');
          }
