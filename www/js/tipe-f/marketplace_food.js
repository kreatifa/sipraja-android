var list_all_food = {
    path: "/tipe-f/marketplace_food/list_all",
    url: "./pages/tipe-f/marketplace_food/index.html",
    name: "list_all_food",
    on: {
        pageInit: function () {
            ...
            app.request.post(site_url_mobile_layanan + "/marketplace_food/kategori_byid", iamthedoor,
                async function (callback) {
                    var list_kategori = '';
                    list_kategori += '<div class="row">';
                    await callback.forEach(function (item, index) {
                        list_kategori += '<div class="col-33">' +
                            '<a href="/tipe-f/marketplace_food/kategori_produk/' + item.id + '/0">' +
                            '<center>' +
                            '<div class="card">' +
            ...
            app.request.get(site_url_mobile_layanan + "/marketplace_food/rekomendasi_product", iamthedoor,
                async function (callback) {
                    app.dialog.close();
...
var kategori_food = {
    path: "/tipe-f/marketplace_food/kategori_produk/:id/:kode",
    url: "./pages/tipe-f/marketplace_food/kategori_produk.html",
    name: "kategori_food",
    on: {
        pageInit: function () {
            var id = mainView.router.currentRoute.params.id;
            var kode = mainView.router.currentRoute.params.kode;
            app.request.get(site_url_mobile_layanan + "/marketplace_food/search/" + id + '/' + kode, iamthedoor,
                function (callback) {
...
var keranjang_food = {
    path: "/tipe-f/marketplace_food/keranjang",
    url: "./pages/tipe-f/marketplace_food/keranjang.html",
    name: "keranjang_food",
    on: {
        pageInit: function () {
            app.request.post(site_url_mobile_layanan + "/marketplace_food/keranjang", iamthedoor,
                function (data) {
                    ...
                    if (data.checkout > 0) {
                        app.dialog.alert('Barang dicheckout anda tidak ditemukan', function () {
                            app.request.post(site_url_mobile_layanan + "/marketplace_food/save_pembatalan/checkout", iamthedoor, function (response) {
                            }, 'json');
                        });
                    }
                    ...
                    if (data.keranjang != '') {
                        ...
                        $.each(groupByCategory, function (key, value) {
                            ...
                            for (var i = 0; i < value.length; i++) {
                                ...
                                <label class="item-content">
                                    ...
                                    <a href="/tipe-f/product/edit_food_keranjang/${value[i].id_product}/${value[i].id}/${value[i].id_product_item}" class="item-link item-content">
                        ...
                        keranjang += '<center>Tidak ada food</center>';
                        $('#beli').remove();
                    }
            ...
            $$('#beli').on('click', function () {
                if ($$('input[type="checkbox"]:checked').length < 1) {
                    app.dialog.alert('Mohon pilih food terlebih dahulu');
                }
                ...
                app.request.post(site_url_mobile_layanan + '/marketplace_food/save_keranjang', ajax_data, function (data) {
                    if (data.status) {
                        app.dialog.alert(data.message, function () {
                            mainView.router.navigate('/tipe-f/marketplace_food/checkout');
                        });
                    } else {
                        app.dialog.alert(data.message, function () {
...
var checkout_food = {
    path: "/tipe-f/marketplace_food/checkout",
    url: "./pages/tipe-f/marketplace_food/checkout.html",
    name: "checkout_food",
    on: {
        pageInit: function () {
            app.dialog.preloader('Loading...');
            app.request.post(site_url_mobile_layanan + "/marketplace_food/checkout", iamthedoor, function (data) {
                app.dialog.close();
                var alamat = '';
                if (data.alamat != null) {
                ...
                $$('#maps').attr('href', '/tipe-f/marketplace_food/maps/' + data.alamat.id);
            ...
            let modal_pembayaran = app.sheet.create({
                content:
                          ...
                          <div class="list no-margin">
                            <div class="row">
                                <div class="col">
                                    <div class="row">
                                        <div class="col-50">
                                            <p style="margin-left: 20px;">Kode Bayar(VA)</p>
                                        </div>
                                        <div class="col-50">
                                            <p id="virtual_account" style="font-weight: bold;"></p>
                                        </div>
                
                                        <div class="col-50">
                                            <p style="margin-left: 20px;">Tanggal Expired</p>
                                        </div>
                                        <div class="col-50">
                                            <p id="tanggal_expired" style="font-weight: bold;"></p>
                                        </div>
                
                                        <div class="col-100">
                                            <div class="col-50" style="margin:auto">
                                                <button id="download_invoice" class="col button button-fill color-green">
            ...
            $$('#invoice_pembayaran').on('click', function () {
                modal_pembayaran.open();
            });

            $$('#pembatalan').on('click', function () {
                app.dialog.confirm('Apakah Anda yakin membatalkan pesanan?', function () {
                    app.request.post(site_url_mobile_layanan + '/marketplace_food/save_pembatalan/checkout', iamthedoor, function (data) {
            ...
            $$('#save_checkout').on('click', function () {
                ...
                app.request.post(site_url_mobile_layanan + '/marketplace_food/save_pesanan', ajax_data, function (data) {
                    app.dialog.close();
                    app.dialog.alert(data.message, async function () {
                        if (data.status) {
                            $('.pesan').hide();
                            $('.bayar').show();
                            modal_pembayaran.open();
                            $('#virtual_account').text(data.return.virtual_account);
                            $('#tanggal_expired').text(data.return.tanggal_expired);
                            $('#download_invoice').off().on('click', function () {
                                app.dialog.close();
                                app.dialog.preloader('Loading...');
                                download_doc(data.return.invoice_path + '_signed.pdf');
                            });
                        }
                    });
                }, 'json');
...
var menu_food = {
    path: "/tipe-f/marketplace_food/menu",
    url: "./pages/tipe-f/marketplace_food/menu.html",
    name: "menu_food",
    on: {
        pageInit: function () {
            app.request.post(site_url_mobile_layanan + '/marketplace_food/menu', iamthedoor, function (response) {
                var menu = '';
                const fieldMenu = response.fieldMenu;
                const fieldImage = response.fieldImage;
                for (key in fieldImage) {
                    menu += `<div class="col-50">
                        <a href="/tipe-f/marketplace_food/list_menu/${fieldImage[key]}" class="link icon-only">
                            <article class="photo">
...
var list_menu_food = {
    path: "/tipe-f/marketplace_food/list_menu/:kode",
    url: "./pages/tipe-f/marketplace_food/list_menu.html",
    name: "list_menu_food",
    on: {
        pageInit: function () {
            ...
            function reload_list() {
                app.request.post(site_url_mobile_layanan + '/marketplace_food/list_menu/' + kode, iamthedoor, function (response) {
                    ...
                    if (response.pesanan.length == 0) {
                        ...
                        response.pesanan.forEach(function (item, index) {
                            var buttonAksi = '';
                            var komplain = '';
                            if (kode == 'belum_bayar' && item.status_bayar != 1) {
                                ...
                                if (item.status == 3) {
                                    if (item.id_komplain === null) {
                                        komplain = `<div class="col">
                                            <a href="/tipe-f/marketplace_food/komplain/${item.id}/${item.kode_toko}" class="button button-fill button-small color-red">
                                                Komplain
                                            </a>
                                        </div>`;
                                    }
                                    buttonAksi = `<div class="col">
                                            <a href="/tipe-f/marketplace_food/return/${item.id}" class="button button-fill button-small color-yellow">
                                                Return
                                            </a>
                                        </div>
                                        ${komplain}
                                        <div class="col">
                                            <a href="/tipe-f/marketplace_food/penilaian/${item.id}" class="button button-fill button-small color-green">
                                                Nilai
                                            </a>
                                        </div>`;
                                }
                                ...
                                buttonAksi = `<div class="col">
                                        <a href="/tipe-f/packing/edit_komplain/${item.id_komplain}/marketplace_food" class="button button-fill button-small color-blue">
                                            Lihat
                                        </a>
                                    </div>`;
                            } else if (kode == 'refund') {
                            ...
                            <div class="card" id="list_menu-${item.id}" style="background-color: ghostwhite">
                                <div class="card-content card-content-padding">
                                    <a href="/tipe-f/marketplace_food/tracking_pesanan/${item.id}/${status}" class="item-link item-content">
                                        <div class="item-media"><img src="${site_url_image_layanan + item.foto}" width="80"/></div>
                                        <div class="item-inner">
                                            <div class="item-title">
                                                <div class="item-title"><h4>${item.nama}</h4></div>
                                            </div>
                                            ...
                                            <div class="item-text"><span style="color:${item.status_bayar_color}">${item.status_bayar_desc}</span></div>
                                        </div>
                                    </a>
                                    <div class="row">
                                        ${buttonAksi}
                        ...
                        on: {
                            open: function () {
                                ...
                                $$('#form_pembatalan_pesanan #save_pembatalan').on('click', function () {
                                    ...
                                    app.request.post(site_url_mobile_layanan + '/marketplace_food/save_pembatalan/pesanan', ajax_data, function (response) {
...
var penilaian_food = {
    path: "/tipe-f/marketplace_food/penilaian/:product_id",
    url: "./pages/tipe-f/marketplace_food/penilaian.html",
    name: "penilaian_food",
    on: {
        pageInit: function () {
            let product_id = mainView.router.currentRoute.params.product_id;
            app.request.post(site_url_mobile_layanan + "/marketplace_food/penilaian_produk/" + product_id, iamthedoor, function (resp) {
            ...
            $('#simpan-penilaian').on('click', function () {
                if ($('#marketplace_food_penilaian_produk')[0].checkValidity() == true) {
                    app.dialog.preloader();
                    let form_data = app.form.convertToData('#marketplace_food_penilaian_produk');

                    let ajax_data = new Array();
                    ajax_data.push(iamthedoor);
                    ajax_data.push(form_data);
                    app.request.post(site_url_mobile_layanan + '/marketplace_food/save_penilaian', ajax_data, function (resp) {
                        app.dialog.close();
                        if (resp.status) {
                            app.dialog.alert(resp.message, function () {
                                mainView.router.navigate('/tipe-f/marketplace_food/menu');
                            });
                        } else {
...
var komplain_food = {
    path: "/tipe-f/marketplace_food/komplain/:product_id/:kode_toko",
    url: "./pages/tipe-f/marketplace_food/komplain.html",
    name: "komplain_food",
    on: {
        pageInit: function () {
            app.dialog.preloader();
            ...
            app.request.post(site_url_mobile_layanan + '/marketplace_food/get_kategori_komplain', iamthedoor, function (response) {
                app.dialog.close();
            ...
            $('#simpan-komplain').on('click', function () {
                if ($('#marketplace_food_komplain_produk')[0].checkValidity() == true) {
                    app.dialog.preloader();
                    let form_data = app.form.convertToData('#marketplace_food_komplain_produk');

                    let ajax_data = new Array();
                    ajax_data.push(iamthedoor);
                    ajax_data.push(form_data);
                    app.request.post(site_url_mobile_layanan + '/marketplace_food/save_komplain', ajax_data, function (resp) {
                        app.dialog.close();
                        if (resp.status) {
                            app.dialog.alert(resp.message, function () {
                                mainView.router.navigate('/tipe-f/marketplace_food/list_menu/komplain');
                            });
                        } else {
                            app.dialog.alert(resp.message);
                        }
...
var pengembalian_food = {
    path: "/tipe-f/marketplace_food/return/:id",
    url: "./pages/tipe-f/marketplace_food/return.html",
    name: "return_food",
    on: {
        pageInit: function () {
            ...
            app.dialog.preloader('Loading...');
            $('#id').val(id);
            app.request.post(site_url_mobile_layanan + "/marketplace_food/return/" + id, iamthedoor, function (response) {
                app.dialog.close();
                response.pesanan.forEach(function (item, index) {
            ...
            $('#simpan-return').on('click', function () {
                if ($('#marketplace_food_return')[0].checkValidity() == true) {
                    app.dialog.preloader();
                    let form_data = app.form.convertToData('#marketplace_food_return');

                    let ajax_data = new Array();
                    ajax_data.push(iamthedoor);
                    ajax_data.push(form_data);
                    app.request.post(site_url_mobile_layanan + '/marketplace_food/save_return', ajax_data, function (resp) {
                        app.dialog.close();
                        if (resp.status) {
                            app.dialog.alert(resp.message, function () {
                                mainView.router.navigate('/tipe-f/marketplace_food/list_menu/pengembalian');
                            });
                        } else {
                            app.dialog.alert(resp.message);
                        }
...
var maps_checkout_food = {
    name: 'maps_checkout_food',
    path: '/tipe-f/marketplace_food/maps/:id',
    url: './pages/tipe-f/marketplace_food/maps.html',
    on: {
        pageInit: function () {
            ...
            if (!maplet) {
                app.request.post(site_url_mobile_layanan + "/marketplace_food/maps/" + id, iamthedoor, function (data) {
...
var tracking_pesanan_food = {
    path: "/tipe-f/marketplace_food/tracking_pesanan/:id/:kode",
    url: "./pages/tipe-f/marketplace_food/tracking_pesanan.html",
    name: "tracking_pesanan_food",
    on: {
        pageInit: function () {
            let id = mainView.router.currentRoute.params.id;
            let kode = mainView.router.currentRoute.params.kode;
            ...
            app.dialog.preloader('Loading...');
            app.request.post(site_url_mobile_layanan + "/marketplace_food/detail_pesanan/" + id + '/' + kode, iamthedoor, function (data) {
                app.dialog.close();
                ...
                $$('#kode_biling').html(data.pesanan.kode_biling);
                $$('#tanggal_expired').html(data.pesanan.tanggal_expired);
                $$('#status_bayar_desc').html(data.pesanan.status_bayar_desc);
                $$('#status_bayar_desc').css('color', data.pesanan.status_bayar_color);
                ...
                $$('#download_invoice').on('click', function () {
                    download_doc(data.pesanan.invoice_path + '_signed.pdf');
                })
...
function checkall(kode_toko) {
    app.request.post(site_url_mobile_layanan + "/marketplace_food/get_keranjang_toko/" + kode_toko, iamthedoor, function (data) {
...
function delete_keranjang(id) {
    app.request.post(site_url_mobile_layanan + '/marketplace_food/delete_keranjang/' + id, iamthedoor, function (response) {
        if (response.success) {
            $$('#keranjang-' + id).remove();
