...
function send_dtks() {
  const encodedNik = btoa($("input[name='nik_pemohon']").val()).replace(/=/g, '');
  app.request.post(site_url_mobile_layanan + '/sktm_kecamatan/cek_dtks/' + encodedNik, iamthedoor, function (response) {
    if (response.jumlah == 0) {
      app.dialog.confirm('Data NIK Anda Tidak Terdaftar di DTKS! Apakah Anda Yakin untuk Melanjutkan?', function () {
        $('#tipe_dtks').val(0);
      }, function () {
...
var new_sktm_kecamatan = {
  ...
  on: {
    pageInit: function () {
      ...
      $(".skmm_group").hide();
      $(".default_upload").show();
      get_faskes('#id_faskes', '', 'create');

      app.request.post(site_url_mobile_layanan + '/sktm_kecamatan/get_berkas', iamthedoor, function (result) {
        ...
        if ($$('#new_sktm_kecamatan')[0].checkValidity() == true) {
          ...
          $('.filecode').each(function (i, el) {
            if (jenis_surat == 'jkmm' && jenis_layanan == 'Rawat Inap') {
              if ((i == 0 && !el.value) || (i == 1 && !el.value) || (i == 2 && !el.value) || (i == 3 && !el.value)) {
                app.dialog.alert('Harap Isi file pada pilihan Rawat Inap');
                cek_file = false;
                return false;
              }
            } else if (jenis_surat == 'jkmm' && jenis_layanan == 'Rawat Jalan') {
              if (i != 5 && !el.value) {
          ...
          $('input[name^=keteranganid]').each(function (i, el) {
          ...
          data.push(filecode);
          app.request.post(site_url_mobile_layanan + '/sktm_kecamatan/save_sktm_kecamatan', data, function (data) {
            if (isNaN(data)) {
  ...
  on: {
    pageInit: function () {
      ...
      var statusselect = app.smartSelect.create({
        ...
        on: {
          close: function () {
            datatables.context[0].ajax.url = site_url_mobile_layanan + '/sktm_kecamatan/layanan/' + $$('#statusselect').val();
      ...
      var datatables = $('#datatables').DataTable({
        ...
        "rowCallback": function (row, data) {
          ...
          if (data.val_status == 'Menunggu') {
            if (datauser.role_id == '4') {
              $('td:eq(0)', row).html('<a href="/tipe-b/edit_sktm_kecamatan/' + data.id + '/edit/" class="button button-small button-fill color-blue">' + '<i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Edit</a>');
            } else {
              $('td:eq(0)', row).html('<a href="/tipe-b/edit_sktm_kecamatan/' + data.id + '/approve/" class="button button-small button-fill color-green">' + '<i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Approve</a>');
            }
          } else {
            $('td:eq(0)', row).html('<a href="/tipe-b/edit_sktm_kecamatan/' + data.id + '/lihat/" class="button button-small button-fill color-green">' + '<i class="icon f7-icons" style="font-size: 12pt;">zoom_in</i> Lihat</a>');
          }
  ...
  on: {
    pageInit: function () {
      ...
      var datatablesperpanjangan = $("#datatablesperpanjangan").DataTable({
        ...
        rowCallback: function (row, data) {
          if (data.jumlah_perpanjangan == 0) {
            $("td:eq(6)", row).html('<span style="background-color:transparent; padding:5px; border-radius:10px; ">Belum <br>diperpanjang</span>');
          } else {
            $("td:eq(6)", row).html(
              '<span style="background-color:transparent; padding:5px; border-radius:10px; ">Sudah <br>diperpanjang</span>'
            );
          }

          if (datauser.role_id == "4") {
            $("td:eq(0)", row).html('<button type="submit" class="button button-fill color-blue perpanjagan" data-id="' + data.id + '">' + '<i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Perpanjang</button>');
          }
        },
      });
      ...
      $("#datatablesperpanjangan").on("click", ".perpanjagan", function () {
        var id = $(this).data("id");
        app.dialog.confirm("Apakah anda ingin memperpanjang sktm?", function () {
          app.request.get(site_url_mobile_layanan + "/sktm_kecamatan/get_id_perpanjangan/2/" + id, iamthedoor, function (data) {
            var tes = data;
            ...
            data.push(approval);
            app.request.post(site_url_mobile_layanan + "/sktm_kecamatan/save_perpanjangan", data, function (data) {
...
var edit_sktm_kecamatan = {
  ...
  on: {
    pageInit: function () {
      ...
      app.request.post(site_url_mobile_layanan + '/sktm_kecamatan/find_layanan/' + id + '/' + datauser.bf_users_id, data, function (data) {
        if (data == false) {
          app.dialog.close();
          ...
          app.request.post(site_url_mobile + '/siurban_mobile/get_kecamatan_kelurahan/' + data.pemohon.kecamatan + '/' + data.pemohon.kode_desa, function (keckel) {
            ...
            $$("select[name='jenis_layanan']").val(data.layanan.jenis_layanan).change();
            ...
            if (data.layanan.file_dinsos) {
              ...
              $$("#print_dinsos_button").on("click", function () {
                ...
                app.request.post(site_url_mobile_layanan + '/sktm_dinsos/print_doc/' + id, iamthedoor, function (doc_path) {
                  download_external('dinsos', doc_path);
                }, 'json');
              });
            }
            ...
            if (data.layanan.file_code !== null) {
              if (tipe == 'edit' && (jenis_surat == 'jkmm' || jenis_surat == 'skmm')) {
                if (jenis_surat == 'jkmm') {
                  get_berkas_sktm(data.deskripsi_dokumen_jkmm, data.attachments, 'edit')
                } else if (jenis_surat == 'skmm') {
                  get_berkas_sktm(data.deskripsi_dokumen_skmm, data.attachments, 'edit')
                }
              } else if (tipe == 'edit') {
            ...
            $$('#deletelayanan').on('click', function () {
              app.dialog.confirm('Apakah anda yakin menghapus data ini?', function () {
                data = [];
                data.push(iamthedoor);
                app.request.post(site_url_mobile_layanan + '/sktm_kecamatan/delete_layanan/' + id, data, function (data) {
                  if (data == false) {
                    app.dialog.close();
                    app.dialog.alert('Data tidak ditemukan');
                    mainView.router.back();
        ...
        $$("#simpan").on("click", function () {
          ...
          if ($$('#edit_sktm_kecamatan')[0].checkValidity() == true) {
            ...
            if (datauser.role_id == '4') {
              ...
              data.push(keteranganid);
              data.push(filecode);
              var url = site_url_mobile_layanan + '/sktm_kecamatan/save_sktm_kecamatan/update/' + id;
              ...
              if (this_user_is_the_last_index == true && $$("select[name='status']").val() == 2) {
                ...
                $$('#approve_button').on('click', function () {
                  approve('/sktm_kecamatan/save_sktm_kecamatan/ustatus/' + id, this_user_is_the_last_index, $$('#esign').val());
                  approval.close();
                });
              } else {
                var approval = app.popup.create({
                ...
                $$("#approve_button").on("touchend", function () {
                  approve('/sktm_kecamatan/save_sktm_kecamatan/ustatus/' + id, this_user_is_the_last_index);
                  approval.close();
                });
              }
            }
