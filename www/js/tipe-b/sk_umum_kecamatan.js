...
var new_sk_umum_kecamatan = {
  path: '/tipe-b/new_sk_umum_kecamatan/',
  url: './pages/tipe-b/new_sk_umum_kecamatan.html',
  name: 'new_sk_umum_kecamatan',
  on: {
    pageInit: function () {
      add_attachment_pendaftar(datauser.attachments);
      $$("#addformupload").on("touchend", addrow);
      $$("#simpan").on("click", function () {
        app.input.validateInputs("#new_sk_umum_kecamatan");
        if ($$('#new_sk_umum_kecamatan')[0].checkValidity() == true) {
          data = new Array();
          keteranganid = [];
          filecode = [];
          ...
          mydata = app.form.convertToData("#new_sk_umum_kecamatan");
          data.push(mydata);
          data.push(iamthedoor);
          data.push(keteranganid);
          data.push(filecode);
          app.request.post(site_url_mobile_layanan + '/sk_umum_kecamatan/save_sk_umum_kecamatan', data, function (data) {
            if (isNaN(data)) {
...
var sk_umum_kecamatan = {
  path: '/tipe-b/sk_umum_kecamatan',
  url: './pages/tipe-b/sk_umum_kecamatan.html',
  name: 'sk_umum_kecamatan',
  on: {
    pageInit: function () {
      $$("#btnnew").hide();
      ...
      var statusselect = app.smartSelect.create({
        ...
        on: {
          close: function () {
            datatables.context[0].ajax.url = site_url_mobile_layanan + '/sk_umum_kecamatan/layanan/' + $$('#statusselect').val();
      ...
      var datatables = $('#datatables').DataTable({
        "serverSide": true,
        "ajax": {
          "url": site_url_mobile_layanan + '/sk_umum_kecamatan/layanan/1',
          "data": iamthedoor,
          "type": "GET"
        },
        "columns": [{ "data": "id" }, { "data": "kode_transaksi" }, { "data": "nomor" }, { "data": "tgl_buat" }, { "data": "jenis_surat" }, { "data": "display_name" }, { "data": "val_status", "width": "20%" },],
        ...
        "rowCallback": function (row, data) {
          ...
          if (data.val_status == 'Menunggu') {
            if (datauser.role_id == '4') {
              $('td:eq(0)', row).html('<a href="/tipe-b/edit_sk_umum_kecamatan/' + data.id + '/edit/" class="button button-small button-fill color-blue"><i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Edit</a>');
            } else {
              $('td:eq(0)', row).html('<a href="/tipe-b/edit_sk_umum_kecamatan/' + data.id + '/approve/" class="button button-small button-fill color-green"><i class="icon f7-icons" style="font-size: 12pt;">pencil_circle_fill</i> Approve</a>');
            }
          } else {
            $('td:eq(0)', row).html('<a href="/tipe-b/edit_sk_umum_kecamatan/' + data.id + '/lihat/" class="button button-small button-fill color-green"><i class="icon f7-icons" style="font-size: 12pt;">zoom_in</i> Lihat</a>');
          }
          if (!data.nomor) {
            $('td:eq(2)', row).html('---');
          }
          if (data.jenis_surat == 'sk_miskin') {
            $('td:eq(4)', row).html('Surat Keterangan Miskin');
          } else {
            $('td:eq(4)', row).html('Surat Keterangan');
          }
        }
      });
    }
...
var edit_sk_umum_kecamatan = {
  path: '/tipe-b/edit_sk_umum_kecamatan/:id/:tipe',
  url: './pages/tipe-b/edit_sk_umum_kecamatan.html',
  name: 'edit_sk_umum_kecamatan',
  on: {
    pageInit: function () {
      tablename = "kt_sk_belum_kawin";
      $$("#addformupload").hide();
      ...
      $$("#print_button").on("click", function () {
        ...
        app.request.post(site_url_mobile_layanan + '/sk_umum_kecamatan/print_doc/' + id, iamthedoor, function (doc_path) {
      ...
      app.request.post(site_url_mobile_layanan + '/sk_umum_kecamatan/find_layanan/' + id + '/' + datauser.bf_users_id, data, function (data) {
        if (data == false) {
          app.dialog.close();
          ...
          app.request.post(site_url_mobile + '/siurban_mobile/get_kecamatan_kelurahan/' + data.pemohon.kecamatan + '/' + data.pemohon.kode_desa, function (keckel) {
            ...
            if (datauser.role_id == '4') {
              ...
              if (tipe == 'edit') {
                ...
                $$('#edit_sk_umum_kecamatan input').prop("disabled", true);
                $$('#edit_sk_umum_kecamatan textarea').prop("disabled", true);
            ...
            $$('#deletelayanan').on('click', function () {
              app.dialog.confirm('Apakah anda yakin menghapus data ini?', function () {
                data = [];
                data.push(iamthedoor);
                app.request.post(site_url_mobile_layanan + '/sk_umum_kecamatan/delete_layanan/' + id, data, function (data) {
                  if (data == false) {
                    app.dialog.close();
                    app.dialog.alert('Data tidak ditemukan');
                    mainView.router.back();
      ...
      function prep_penyimpanan() {
        $$("#simpan").on("click", function () {
          app.input.validateInputs("#edit_sk_umum_kecamatan");
          if ($$('#edit_sk_umum_kecamatan')[0].checkValidity() == true) {
            data = new Array();
            if (datauser.role_id == '4') {
              ...
              mydata = app.form.convertToData("#edit_sk_umum_kecamatan");
              data.push(mydata);
              data.push(iamthedoor);
              data.push(keteranganid);
              data.push(filecode);
              var url = site_url_mobile_layanan + '/sk_umum_kecamatan/save_sk_umum_kecamatan/update/' + id;
              ...
              if (this_user_is_the_last_index == true && $$("select[name='status']").val() == 2) {
                ...
                $$('#approve_button').on('click', function () {
                  approve('/sk_umum_kecamatan/save_sk_umum_kecamatan/ustatus/' + id, this_user_is_the_last_index, $$('#esign').val());
                  approval.close();
                });
              } else {
                var approval = app.popup.create({
                ...
                $$("#approve_button").on("touchend", function () {
                  approve('/sk_umum_kecamatan/save_sk_umum_kecamatan/ustatus/' + id, this_user_is_the_last_index);
                  approval.close();
                });
              }
            }
