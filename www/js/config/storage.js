...
Attachment.openDatabase = function () {
  ...
  // if (!localStorage.db_ready) {
  db.transaction(function (tx) {
    let fields = [
      'id',
      'name'
    ];
    tx.executeSql('CREATE TABLE IF NOT EXISTS kt_reg_provinces (' +
      fields[0] + ' INTEGER PRIMARY KEY, ' + fields[1] +
      ')');
    get_kt_reg_provinces().forEach(function (row) {
      tx.executeSql('INSERT OR IGNORE INTO kt_reg_provinces VALUES (?1, ?2)', [
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS kt_reg_regencies (' +
      fields[0] + ' INTEGER PRIMARY KEY, ' + fields[1] + ' INTEGER, ' + fields[2] +
      ')');
    get_kt_reg_regencies().forEach(function (row) {
      tx.executeSql('INSERT OR IGNORE INTO kt_reg_regencies VALUES (?1, ?2, ?3)', [
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS kt_reg_districts (' +
      fields[0] + ' INTEGER PRIMARY KEY, ' + fields[1] + ' INTEGER, ' + fields[2] +
      ')');
    get_kt_reg_districts().forEach(function (row) {
      tx.executeSql('INSERT OR IGNORE INTO kt_reg_districts VALUES (?1, ?2, ?3)', [
        row[fields[0]], row[fields[1]], row[fields[2]]
      ]);
    });
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS kt_reg_villages (' +
      fields[0] + ' INTEGER PRIMARY KEY, ' + fields[1] + ' INTEGER, ' + fields[2] +
      ')');
    get_kt_reg_villages().forEach(function (row) {
      tx.executeSql('INSERT OR IGNORE INTO kt_reg_villages VALUES (?1, ?2, ?3)', [
        row[fields[0]], row[fields[1]], row[fields[2]]
      ]);
    });
...
Attachment.getDbWhere = function ({ table = "", select = "*", join = [], where = [], order_by = "id DESC" }, result) {
  db.transaction(function (tx) {
