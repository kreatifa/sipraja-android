var pembukaan_kredit_bpr = {
  path: '/tipe-d/pembukaan_kredit_bpr/',
  url: './pages/tipe-d/bpr_delta_artha/pembukaan_kredit_bpr.html',
  name: 'pembukaan_kredit_bpr',
  on: {
    pageInit: function () {
      $$('#btnnew').hide();
      ...
      var datatables = $('#datatables').DataTable({
        serverSide: true,
        ajax: {
          "url": site_url_mobile_layanan + '/pembukaan_kredit_bpr/get_data/' + $$('#statusselect').val(),
          "data": iamthedoor,
          "type": "GET"
        },
        columns: [
        ...
        rowCallback: function (row, data) {
          $('td:eq(0)', row).html('<a href="/tipe-d/pembukaan_kredit_bpr_edit/' + data.id + '/edit/" class="button button-small button-fill color-blue">' +
      ...
      var statusselect = app.smartSelect.create({
        ...
        on: {
          close: function () {
            app.dialog.preloader('Loading...');
            datatables.context[0].ajax.url = site_url_mobile_layanan + '/pembukaan_kredit_bpr/get_data/' + $$('#statusselect').val();
...
var pembukaan_kredit_bpr_new = {
  path: '/tipe-d/pembukaan_kredit_bpr_new/',
  url: './pages/tipe-d/bpr_delta_artha/pembukaan_kredit_bpr_new.html',
  name: 'pembukaan_kredit_bpr_new',
  on: {
    pageInit: function () {
      ...
      app.dialog.preloader('Loading...');
      app.request.post(site_url_mobile_layanan + '/pembukaan_kredit_bpr/get_list_lampiran', iamthedoor, function (data) {
        app.dialog.close();
      ...
      $$('#simpan').on('click', function () {
        app.input.validateInputs('#pembukaan_kredit_bpr_new');
        // if ($$('#pembukaan_kredit_bpr_new')[0].checkValidity() == true) {

        let form_data = app.form.convertToData('#pembukaan_kredit_bpr_new');
        ...
        app.request.post(site_url_mobile_layanan + '/pembukaan_kredit_bpr/cek_data', ajax_data, function (cek) {
          if (cek.count > 0) {
            app.dialog.alert('Anda telah memiliki produk kredit yang sama. Silakan mengajukan produk yang lain');
          } else {
            app.request.post(site_url_mobile_layanan + '/pembukaan_kredit_bpr/save_pembukaan_kredit_bpr', ajax_data, function (data) {
...
var pembukaan_kredit_bpr_edit = {
  path: '/tipe-d/pembukaan_kredit_bpr_edit/:id/:tipe',
  url: './pages/tipe-d/bpr_delta_artha/pembukaan_kredit_bpr_edit.html',
  name: 'pembukaan_kredit_bpr_edit',
  on: {
    pageInit: function () {
      ...
      app.dialog.preloader('Loading...');
      app.request.post(site_url_mobile_layanan + '/pembukaan_kredit_bpr/find_layanan/' + id, iamthedoor, function (callback) {
        app.dialog.close();
        ...
        $$('#pinjaman').val(callback.layanan.pinjaman);
        $$('#lama_pinjaman').val(callback.layanan.lama_pinjaman);
        $$('#jenis_jaminan').val(callback.layanan.jenis_jaminan);
        $$('#rincian_jaminan').val(callback.layanan.rincian_jaminan);

        $$('#kode_produk').prop('disabled', true);
        get_kode_kredit('#kode_produk', callback.layanan.kode_produk);
